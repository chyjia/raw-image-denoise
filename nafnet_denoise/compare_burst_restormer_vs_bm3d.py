"""Compare BurstRestormer-16f, Adaptive NAFNet, and PTC VST+BM3D."""
...
from .infer_burst import denoise_burst, load_burst_restormer, prepare_aligned_burst
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("nafnet_denoise/compare_burst_restormer_vs_bm3d"),
    )
    parser.add_argument(
    ...
    parser.add_argument("--checkpoint-residual-4f", type=Path, default=None)
    ...
    restormer, _width, input_frames = load_burst_restormer(
        args.checkpoint_burst_restormer,
        device,
    )
    ...
    for path in files:
        ...
        print(f"{path.name}: frame {target_index} burst_restormer...", flush=True)
        aligned = prepare_aligned_burst(frames, target_index, input_frames)
        restormer_dn = denoise_burst(
            restormer,
            aligned,
            exposure_ms,
            device,
        ...
        outputs = {
            ...
            "vst_bm3d": bm3d_dn,
            "burst_restormer": restormer_dn,
        }
        labels = ["Input", "Temporal ref", "VST+BM3D", "BurstRestormer"]
        if adaptive_dn is not None:
            outputs["adaptive"] = adaptive_dn
            labels.append(f"Adaptive {adaptive_meta.get('route', '')}")
        ...
        for method, image in outputs.items():
            if method == "burst_restormer":
                row = metric_row(
                    ...
                    ys,
                    xs,
                    checkpoint=str(args.checkpoint_burst_restormer),
                ...
                row = metric_row(
                    ...
                    input_frames=int(adaptive_meta.get("used_frames", 0)),
                    alignment_response=float(
                        adaptive_meta.get("alignment_confidence", 1.0)
                    ),
                    fallback_used=bool(adaptive_meta.get("fallback_used", False)),
                )
