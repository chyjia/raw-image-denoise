"""P4-d: distill P4-a blend teacher (SOTA↔lap_ms, T=8, harden=16) → single DualHead.

Student resumes split_edge_fid under fps-gated FE. Goal: single-net mean DES ≥ 0.9060.
"""
...
def main() -> None:
    ...
    edgekd = Path("nafnet_denoise/checkpoints_4f_lap_edge_ms/best.pt")
    out_dir = Path("nafnet_denoise/checkpoints_4f_blend_distill_p4")
    compare_dir = Path("nafnet_denoise/compare_blend_distill_p4")

    if not resume.exists():
        raise SystemExit(f"Missing resume: {resume}")
    if not edgekd.exists():
        raise SystemExit(f"Missing edge arm teacher: {edgekd}")
    ...
    if not args.skip_compare:
        ...
        print(
            "refs: P4-a blend T8_h16 0.9060 / gated SOTA 0.9050 / "
            "goal single-net mean>=0.9060",
            flush=True,
        )
