"""Chain P101+ after P98 Anscombe bake; early-stop on consecutive no-bake."""
...
MODULES = [
    ("101", "nafnet_denoise.run_des_meta100_p101"),
    ("102", "nafnet_denoise.run_des_meta100_p102"),
    ("103", "nafnet_denoise.run_des_meta100_p103"),
]

EXTRA_TEMPLATES = [
    "run_des_meta100_p101.py",
    "run_des_meta100_p102.py",
    "run_des_meta100_p103.py",
]


def current_baseline(root: Path) -> float:
    for cyc in range(130, 39, -1):
    ...
    return float(m.group(1)) if m else 0.9472
...
def main() -> None:
    ...
    log_path = log_dir / "cycle_log_p101plus.jsonl"
    ...
    for cyc, mod in MODULES:
        ...
        if rec["baked"] and rec["best"] is not None and float(rec["best"]) > baseline + 1e-4:
            ...
            stagnant = 0
            # sync deploy defaults from latest hook if present
            hook = root / f"deploy_p{cyc}_hook.json"
            if hook.exists():
                try:
                    r = json.loads(hook.read_text(encoding="utf-8")).get("recipe") or {}
                    dep = root / "infer_blend_deploy.py"
                    text = dep.read_text(encoding="utf-8")
                    s = r.get("strength", r.get("ans_s"))
                    k = r.get("k_mad", r.get("ans_k"))
                    sig = r.get("sigma", r.get("ans_sig"))
                    fp = r.get("flat_pct", r.get("ans_fp"))
                    if s is not None:
                        text = re.sub(
                            r"ans_strength: float = [0-9.]+",
                            f"ans_strength: float = {float(s):g}",
                            text,
                            count=1,
                        )
                    if k is not None:
                        text = re.sub(
                            r"ans_k_mad: float = [0-9.]+",
                            f"ans_k_mad: float = {float(k):g}",
                            text,
                            count=1,
                        )
                    if sig is not None:
                        text = re.sub(
                            r"ans_sigma: float = [0-9.]+",
                            f"ans_sigma: float = {float(sig):g}",
                            text,
                            count=1,
                        )
                    if fp is not None:
                        text = re.sub(
                            r"ans_flat_pct: float = [0-9.]+",
                            f"ans_flat_pct: float = {float(fp):g}",
                            text,
                            count=1,
                        )
                    dep.write_text(text, encoding="utf-8")
                except Exception as exc:
                    print(f"deploy sync warn: {exc}", flush=True)
        else:
            stagnant += 1
