"""Chain P56+ DES cycles; early-stop after consecutive no-bake (default 5)."""
...
MODULES = [
    ("56", "nafnet_denoise.run_des_meta100_p56"),
    ("57", "nafnet_denoise.run_des_meta100_p57"),
    ("58", "nafnet_denoise.run_des_meta100_p58"),
    ("59", "nafnet_denoise.run_des_meta100_p59"),
]

EXTRA_TEMPLATES = [
    "run_des_meta100_p56.py",
    "run_des_meta100_p57.py",
    "run_des_meta100_p59.py",
    "run_des_meta100_p58.py",
]
...
def main() -> None:
    ...
    log_path = log_dir / "cycle_log_p56plus.jsonl"
    ...
    for i in range(int(args.extra_cycles)):
        ...
        if not script.exists():
            ...
            # rename cycle markers
            src_num = re.search(r"p(\d+)\.py", tmpl.name).group(1)
            text = text.replace(f"P{src_num}", f"P{cyc}").replace(f"p{src_num}", f"p{cyc}")
            text = re.sub(r"BASELINE = [0-9.]+", f"BASELINE = {baseline:.6f}", text)
            text = text.replace(f"compare_meta100_p{src_num}", f"compare_meta100_p{cyc}")
            text = text.replace(f"deploy_p{src_num}_hook", f"deploy_p{cyc}_hook")
            # nudge pad grids
            text = text.replace("(i % 8)", f"((i + {cyc}) % 8)")
            text = text.replace("(i % 5)", f"((i + {cyc}) % 5)")
