"""P129: EUI recursive spin + interscale Haar (full GAT pipeline lite).

Baseline: P120 DES ≈ 0.9556
Makitalo–Foi EUI + Luisier interscale on flats after P126 core.
"""
...
from .p8_fusion import deploy_p120, deploy_p129
...
def build_recipes() -> list[Recipe]:
    ...
    for rs in [0.0, 0.01, 0.02, 0.03]:
        for ist in [0.08, 0.12, 0.15, 0.2]:
            for ik in [0.75, 1.0, 1.25]:
                for nit in [2, 3]:
                    for decay in [0.5, 0.7]:
                        for k in [1.0, 1.2]:
                            recipes.append(
                                Recipe(
                                    name=f"full_rs{rs:g}_is{ist:g}_ik{ik:g}_n{nit}",
                                    family="full_gat",
                                    read_sigma=rs,
                                    is_strength=ist,
                                    is_k_mad=ik,
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p129_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P129 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p129")
    )
