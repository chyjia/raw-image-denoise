"""P134: SURE-lite k_mad selection on recursive EUI (Luisier TIP'07).

Baseline: P126 DES ≈ 0.9565
Minimize flat residual risk + 2σ²·keep_frac over k candidates.
"""
...
from .p8_fusion import deploy_p126, deploy_p134
...
class Recipe:
    name: str
    family: str
    k_list: tuple[float, ...] = (0.8, 1.0, 1.2, 1.4)
    n_iters: int = 3
    thr_decay: float = 0.7
    sigma: float = 2.4
            ...
            for decay in [0.5, 0.7, 0.85]:
                for sig in [2.0, 2.4, 2.8]:
                    for rs in [0.35, 0.4, 0.45]:
                        recipes.append(
                            Recipe(
                                name=f"sure_{'_'.join(f'{k:g}' for k in ks)}_n{nit}_d{decay:g}",
                                family="sure",
                                k_list=ks,
                                n_iters=nit,
                                thr_decay=decay,
...
def apply_recipe(recipe: Recipe, cache_dir: Path, stem: str, fps: float):
    ...
    return deploy_p134(sota, edge, fps, k_list=recipe.k_list, **kw)


def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p134_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P134 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p134")
    )
