"""P144: non-negative garrote + SURE-k (Gao/Breiman WaveShrink).

Baseline: P134 DES ≈ 0.9572
Replace soft threshold with garrote inside EUI recursive spin; SURE pick k.
"""
...
from .p8_fusion import deploy_p134, deploy_p140
            ...
            for decay in [0.5, 0.7, 0.85]:
                for sig in [1.8, 2.0, 2.4]:
                    for rs in [0.3, 0.35, 0.4]:
                        recipes.append(
                            Recipe(
                                name=f"gar_{'_'.join(f'{k:g}' for k in ks)}_n{nit}_d{decay:g}",
                                family="garrote",
                                k_list=ks,
                                n_iters=nit,
                                thr_decay=decay,
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p144_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P140 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p144")
    )
