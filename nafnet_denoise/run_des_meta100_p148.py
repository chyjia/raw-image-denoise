"""P148: overlapping NeighShrink James–Stein (Chen NeighSure).

Baseline: P142 DES ≈ 0.9580
Replace tiled BlockJS with local energy window (overlapping JS).
"""
...
from .p8_fusion import deploy_p142, deploy_p148
            ...
            for nit in [3, 4]:
                for decay in [0.5, 0.7]:
                    for sig in [1.8, 2.0, 2.4]:
                        recipes.append(
                            Recipe(
                                name=f"ns_w{win}_n{nit}_d{decay:g}_sig{sig:g}",
                                family="neigh",
                                k_list=ks,
                                neigh_win=win,
                                n_iters=nit,
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p148_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P148 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p148")
    )
