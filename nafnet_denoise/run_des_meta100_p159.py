"""P159: SURE pick among BiShrink / NeighLevel / Bayes / BiNeigh shrink modes.
...
from .p8_fusion import deploy_p150, deploy_p159
...
def build_recipes() -> list[Recipe]:
    ...
    mode_sets = [
        ("bishrink", "neighlevel", "bayes", "bineigh"),
        ("bishrink", "neighlevel", "bineigh"),
        ("bishrink", "bayes", "bineigh"),
        ("neighlevel", "bayes", "bineigh"),
    ]
    grids = [
        (0.8, 1.0, 1.2),
        (0.7, 1.0, 1.3),
                ...
                for decay in [0.6, 0.7, 0.85]:
                    for sig in [1.6, 1.8, 2.0]:
                        for rs in [0.28, 0.3, 0.35]:
                            recipes.append(
                                Recipe(
                                    name=f"mp_{tag}_{'_'.join(f'{k:g}' for k in ks)}_n{nit}",
                                    family="multipick",
                                    k_list=ks,
                                    n_iters=nit,
                                    thr_decay=decay,
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p159_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P159 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p159")
    )
