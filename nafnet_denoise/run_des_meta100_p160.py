"""P160: Cho NeighLevel parent-scaled threshold + SURE-k.

Baseline: P150 DES ≈ 0.9589
Parent low-pass modulates local HP threshold in Anscombe domain.
"""
...
from .p8_fusion import deploy_p150, deploy_p156
                    ...
                    for sig in [1.6, 1.8, 2.0]:
                        for rs in [0.28, 0.3, 0.35]:
                            recipes.append(
                                Recipe(
                                    name=(
                                        f"nl_{'_'.join(f'{k:g}' for k in ks)}"
                                        f"_a{alpha:g}_n{nit}_d{decay:g}"
                                    ),
                                    family="neighlevel",
                                    k_list=ks,
                                    n_iters=nit,
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p160_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P156 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p160")
    )
