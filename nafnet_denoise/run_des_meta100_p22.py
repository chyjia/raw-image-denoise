"""P22 meta: tighter schedule around P21 winner (DES ≈ 0.9281).
...
def bake_deploy(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p22_hook.json").write_text(
    ...
    print(f"Baked P22 {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p22")
    )
    ...
    for idx, recipe in enumerate(recipes):
        print(f"\n===== P22 [{idx:03d}/{len(recipes)}] {recipe.name} =====", flush=True)
