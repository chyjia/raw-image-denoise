"""P32 meta: tighter schedule around P21 winner (DES ≈ 0.9281).
    ...
    while len(recipes) < 100:
        ...
        recipes.append(
            Recipe(
                name=f"pad_{i}",
                family="sched",
                u_low=0.18 + ((i + 32) % 6) * 0.01,
                u_mid=0.13 + (i % 5) * 0.01,
                u_high=0.10 + (i % 4) * 0.01,
                low_bilat=0.8 + ((i + 32) % 4) * 0.04,
...
def bake_deploy(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p32_hook.json").write_text(
    ...
    print(f"Baked P32 {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p32")
    )
    ...
    for idx, recipe in enumerate(recipes):
        print(f"\n===== P32 [{idx:03d}/{len(recipes)}] {recipe.name} =====", flush=True)
