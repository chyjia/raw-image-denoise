"""P34 meta: tighter schedule around P21 winner (DES ≈ 0.9281).

Baseline: bilat 0.85/0.8, unsharp 0.20/0.15/0.12 on P19 arms.
"""
...
def build_100_recipes() -> list[Recipe]:
    recipes = [Recipe(name="p21", family="baseline", u_low=0.2, low_bilat=0.85, mid_bilat=0.8)]
    for ul, um, uh in [
        (0.20, 0.15, 0.12),
        (0.21, 0.15, 0.12),
        (0.19, 0.15, 0.12),
    ...
    while len(recipes) < 100:
        ...
        recipes.append(
            Recipe(
                name=f"pad_{i}",
                family="sched",
                u_low=0.18 + ((i + 34) % 6) * 0.01,
                u_mid=0.13 + (i % 5) * 0.01,
                u_high=0.10 + (i % 4) * 0.01,
                low_bilat=0.8 + ((i + 34) % 4) * 0.04,
                mid_bilat=0.75 + (i % 3) * 0.05,
            )
        )
    seen: set[str] = set()
    out: list[Recipe] = []
...
def bake_deploy(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p34_hook.json").write_text(
    ...
    print(f"Baked P34 {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p34")
    )
    ...
    for idx, recipe in enumerate(recipes):
        print(f"\n===== P34 [{idx:03d}/{len(recipes)}] {recipe.name} =====", flush=True)
