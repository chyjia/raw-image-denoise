"""P42: refine P31 fps bands + FE-aligned mid thresholds (zero-train).

Baseline: P31 DES ≈ 0.9292 (low_fps=1.5)
Lit leftovers: fps/σ gated schedules — retune mid_fps / low_fps neighborhood.
"""
...
def build_recipes() -> list[Recipe]:
    ...
    for low in [1.25, 1.5, 1.67, 1.75, 2.0, 2.5]:
        for mid in [4.0, 5.0, 6.0, 7.0, 8.0]:
            recipes.append(
                Recipe(name=f"fps_{low:g}_{mid:g}", family="sched", low_fps=low, mid_fps=mid)
            )
    for h in [32.0, 40.0, 48.0, 56.0]:
        recipes.append(Recipe(name=f"bh_{h:g}", family="sched", bilat_harden=h))
    for ul, um, uh in [
        (0.20, 0.16, 0.12),
        (0.22, 0.16, 0.12),
        (0.20, 0.18, 0.12),
        ...
        for lb, mb in [(0.9, 0.85), (0.92, 0.88), (0.88, 0.85), (0.95, 0.9)]:
            recipes.append(
                Recipe(
                    name=f"sch_u{ul:g}_{um:g}_{uh:g}_b{lb:g}_{mb:g}",
                    family="sched",
                    u_low=ul, u_mid=um, u_high=uh, low_bilat=lb, mid_bilat=mb,
                )
            )
    while len(recipes) < 100:
        ...
        recipes.append(
            Recipe(
                name=f"pad_{i}",
                family="sched",
                low_fps=1.25 + ((i + 42) % 5) * 0.25,
                mid_fps=4.0 + (i % 5) * 1.0,
                bilat_harden=32.0 + (i % 4) * 8.0,
            )
        )
    seen = set()
    out = []
...
def bake_fn(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p42_hook.json").write_text(
    ...
    print(f"Baked P42 {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p42"))
