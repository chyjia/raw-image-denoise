"""P51: SS-TTA / DualEx consistency — pull flats toward FE when |deploy−FE| large.
...
def bake_fn(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p51_hook.json").write_text(
    ...
    Path("nafnet_denoise/compare_meta100_p51").mkdir(parents=True, exist_ok=True)

    Path("nafnet_denoise/compare_meta100_p51/BAKE_NOTE.txt").write_text(
    ...
    print(f"Wrote P51 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p51")
