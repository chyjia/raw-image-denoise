"""P60: DualEx-style HF residual compensation after P40 deploy.
    ...
    while len(recipes) < 100:
        ...
        recipes.append(
            Recipe(
                name=f"pad_hf_{i}",
                family="hf_res",
                amount=0.05 + ((i + 60) % 8) * 0.03,
                sigma=1.0 + ((i + 60) % 5) * 0.2,
                edge_only=(i % 2 == 0),
            )
        )
    seen: set[str] = set()
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p60_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    note = Path("nafnet_denoise/compare_meta100_p60/BAKE_NOTE.txt")
    ...
    print(f"Wrote P60 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p60")
    )
