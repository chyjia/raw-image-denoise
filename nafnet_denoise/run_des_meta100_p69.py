"""P69: refine P64 exposure thresholds / bilat-unsharp (AdaptiveISP-style param search).

Baseline: P64 DES ≈ 0.9382
Lit: AdaptiveISP / MAS-ISP — scene-adaptive ISP hyperparameters.
(Replaces missed P66 exposure refine that raced with a LUT clone.)
"""
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p69_hook.json").write_text(
    ...
    # also sync p8 defaults
    p8 = Path("nafnet_denoise/p8_fusion.py")
    p8t = p8.read_text(encoding="utf-8")
    for key, arg in [
        ("t_dark", "t_dark"),
        ("t_bright", "t_bright"),
        ("bilat_d", "bilat_d"),
    ...
    p8.write_text(p8t, encoding="utf-8")
    dep.write_text(text, encoding="utf-8")
    print(f"Baked P69 into deploy {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p69")
    )
