"""P77: mid_fps / low_fps threshold refine (ISP band boundaries).
...
def bake_fn(best: dict) -> None:
    ...
    Path("nafnet_denoise/deploy_p77_hook.json").write_text(
    ...
    text = re.sub(
        ...
        f"Defaults (P77 fps bands, mean DES ~{best['mean_des']:.4f})",
        text,
        count=1,
    )
    path.write_text(text, encoding="utf-8")
    print(f"Baked P77 {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p77")
    )
