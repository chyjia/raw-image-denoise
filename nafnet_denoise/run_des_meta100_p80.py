"""P80: ensemble MAD+std noise gates (multi-estimator AdaptiveISP).
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p80_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P80 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p80")
    )
