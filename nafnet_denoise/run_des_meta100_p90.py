"""P90: TTT-MIM output-lite blind-spot consistency on P74 deploy (ECCV'24).
...
def bake_fn(best: dict) -> None:
    Path("nafnet_denoise/deploy_p90_hook.json").write_text(
        json.dumps(best, indent=2), encoding="utf-8"
    )
    stamp_deploy_des(float(best["mean_des"]))
    print(f"Wrote P90 hook {best['name']}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("nafnet_denoise/compare_meta100_p90")
    )
