"""P0-b: edge KD from spec_low under fps-gated FE (train/test domain match).

Resume split_edge_fid; train/infer use ``--wiener-fe-schedule fps_sigma``.
Flat teachers stay partitioned BM3D; edges distill toward frozen spec_low.
Compare uses gated FE via checkpoint schedule flag.
"""
...
def main() -> None:
    ...
    parser.add_argument("--edge-kd-mix", type=float, default=0.80)
    parser.add_argument("--train-edge-only", action="store_true")
    ...
    out_dir = Path("nafnet_denoise/checkpoints_4f_edge_kd_new_fe")
    compare_dir = Path("nafnet_denoise/compare_edge_kd_new_fe")
    ...
    if not args.skip_train:
        cmd = [
            ...
            # Fixed ea_γ as fallback; schedule overrides per-clip at train time.
    ...
    if not args.skip_compare:
        ...
        # Gated FE compare (same protocol as P0-a).
        run(
            [
