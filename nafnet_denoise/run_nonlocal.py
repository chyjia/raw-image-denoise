"""Train dual-head NAFNet + bottleneck NonLocal + flat-σ, then DES compare."""
...
def main() -> None:
    ...
    parser.add_argument("--nonlocal-count", type=int, default=1)
    ...
    out_dir = Path("nafnet_denoise/checkpoints_4f_dual_head_nonlocal")
    compare_dir = Path("nafnet_denoise/compare_dual_head_nonlocal")
    ...
    if not args.skip_train:
        ...
        cmd = [
            ...
            "--arch", "dual_head",
            "--use-nonlocal",
            "--nonlocal-count", str(args.nonlocal_count),
    ...
    if not args.skip_compare:
        ...
        if not ckpt.exists():
            raise SystemExit(f"Missing nonlocal checkpoint: {ckpt}")
        if not baseline.exists():
