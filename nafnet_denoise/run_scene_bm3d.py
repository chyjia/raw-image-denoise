"""Scene-weighted DES flat loss + Wiener-domain BM3D flat teacher short FT.

Resume from postmerge_des (no postmerge σ calib). Online VST-BM3D on the
Wiener-merged patch becomes the flat distill teacher; low-fps scenes get higher
...
def main() -> None:
    ...
    resume = Path("nafnet_denoise/checkpoints_4f_postmerge_des/best.pt")
    out_dir = Path("nafnet_denoise/checkpoints_4f_scene_bm3d")
    compare_dir = Path("nafnet_denoise/compare_scene_bm3d")
    ...
    if not args.skip_train:
        ...
        cmd = [
            ...
            "--highpass-weight", "0.75",
            "--flat-boost", "3.5",
            "--flat-sigma-weight", "0.80",
            "--des-flat-sigma-weight", "1.25",
            "--des-noise-gain-weight", "0.95",
