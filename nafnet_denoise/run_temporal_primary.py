"""Fine-tune dual-head+NL with temporal-primary supervision (low BM3D)."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--teacher-mode",
        choices=("temporal", "blend", "partitioned"),
        default="blend",
    )
    parser.add_argument("--bm3d-flat-alpha", type=float, default=0.20)
    ...
    resume = Path("nafnet_denoise/checkpoints_4f_dual_head_nonlocal/best.pt")
    if not resume.exists():
        resume = Path("nafnet_denoise/checkpoints_4f_dual_head_flat_sigma/best.pt")
    ...
    out_dir = Path("nafnet_denoise/checkpoints_4f_temporal_primary")
    compare_dir = Path("nafnet_denoise/compare_temporal_primary")
    ...
    if not args.skip_train:
        ...
        cmd = [
            ...
            "--use-nonlocal",
            "--nonlocal-count", "1",
            "--teacher-mode", args.teacher_mode,
            "--bm3d-flat-alpha", str(args.bm3d_flat_alpha),
            ...
            "--lr", "8e-6",
            "--teacher-weight-start", "0.25",
            "--teacher-weight-end", "0.10",
            "--highpass-weight", "0.40",
            "--flat-boost", "1.5",
            "--flat-sigma-weight", "0.25",
            "--dual-aux-weight", "0.25",
            "--edge-weight", "0.25",
            "--validation-every", "4",
    ...
    if not args.skip_compare:
        ...
        run(
            [
                ...
                "--label-wide-4f", "temporal_primary_4f",
